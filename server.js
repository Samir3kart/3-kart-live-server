...
function makePlayer(username, nickname) {
  return {
    ...
    cards: [cardValue(), cardValue(), cardValue()],
    joinedAt: Date.now(),
    score: null,
    gift: "Parfum"
  };
...
function addPlayer(username, nickname, source="gift") {
...
function revealRound() {
  ...
  for (const p of state.players) {
    p.score = p.cards.reduce((a,b) => a+b, 0);
  }
  ...
  state.status = winners.length === 1
    ? `Qalib: ${winners[0].nickname}`
    : `Heç-heçə: ${winners.map(w => w.nickname).join(", ")}`;
  broadcast();
}
...
io.on("connection", socket => {
  ...
  socket.on("start-round", () => {
    if (state.revealed) resetRound();
    state.status = "Raund başladı — kartlar açıldı";
    revealRound();
  });
  ...
  socket.on("demo-gift", ({username, nickname} = {}) => {
    addPlayer(username || `demo${Date.now()}`, nickname || username || "Demo", "demo");
  });
});
...
app.post("/api/demo/gift", (req,res) => {
  ...
  const added = addPlayer(username || `demo${Date.now()}`, nickname || username || "Demo", "demo");
  res.json({ok:true, added, players:state.players.length});
});
...
async function connectTikTok() {
  if (!TIKTOK_USERNAME) {
    state.status = "TikTok username gözləyir";
    broadcast();
    return;
  }
  ...
  try {
    ...
    tiktok.on(WebcastEvent.GIFT, data => {
      ...
      // "Parfum" hədiyyəsi iştirakçıya bir dəfə qoşulma verir.
      // Streak hədiyyələrində ilk event kifayətdir; son event təkrar sayılmır.
      ...
      addPlayer(username, nickname, "tiktok");
    });

    tiktok.on("connected", () => {
      state.connected = true;
      state.status = "TikTok LIVE qoşuldu";
      broadcast();
    });

    tiktok.on("disconnected", () => {
      state.connected = false;
      state.status = "TikTok bağlantısı kəsildi";
      broadcast();
      scheduleReconnect();
    });
    ...
    tiktok.on("error", err => {
      ...
      state.connected = false;
      state.status = "TikTok bağlantı xətası";
      broadcast();
      scheduleReconnect();
    });
    ...
    state.connected = false;
    state.status = "TikTok LIVE hazır deyil";
    broadcast();
    scheduleReconnect();
  }
}
...
server.listen(PORT, async () => {
  console.log(`3 Kart LIVE running on port ${PORT}`);
  await connectTikTok();
});
